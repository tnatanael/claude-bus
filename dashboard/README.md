...
- **Handoff flow**: colunas `inbox → processing → done` (+ a área de rejeitados). O **inbox** são os pendentes — rode `/bus` no destino de cada card pra processá-los; é **ordenado pelo próximo tick** do cron do destino (quem será pego antes fica no topo). Cada card traz um **countdown** (`⏱ ~Nm`) até o cron do especialista **destino** pegá-lo (dispara a cada 30 min); se o tique esperado passar +5min sem o handoff sair, vira **`⚠ atrasado Xm · retry ~Ym`** — mostra o atraso E quando vem a próxima tentativa (normal quando o lock global estava ocupado no tique; se persistir muito, o cron do destino pode estar parado — rode `/bus` nele). Na visão de **projeto único**, conectores SVG ligam cada resposta ao handoff pai (`in_reply_to`). A coluna `done` mostra só as últimas 24h (máx 20, mais recente no topo) — filtro de exibição, o disco não é tocado.
