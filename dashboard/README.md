...
- **Especialistas registrados**: cada projeto mostra, em chips, os slugs registrados nele (do `names/`) — quem está naquela frente. Um **ponto** no chip indica se o cron daquele especialista está **armado** (verde — `/bus` visto na última ~1h) ou **não confirmado** (cinza — sessão fechada ou cron parado).
