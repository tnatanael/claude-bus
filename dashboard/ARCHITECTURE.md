...
```
`specialists`: especialistas do projeto, cada um `{ slug, status, seenAgeSec, prio }`. **`status`** = cor do chip pela idade do marcador `seen/<sid>` (regravado a cada `/bus`/tique do hook): **`green`** < 6 min, **`yellow`** 6–10 min, **`red`** > 10 min ou nunca visto (offline). Como o cron dispara `/bus` a cada 5 min, um saudável fica sempre verde. **Exceção:** quem **segura o lock** daquele projeto (trabalhando agora) é promovido a **`green`** mesmo com `seen` velho de um turno longo — trabalhando é o oposto de offline. `seenAgeSec` = idade do seen (null se nunca visto). Lidos do `names/` + `seen/` + `.bus-lock`. Os itens do `inbox` carregam **`tick: true`**, **`toStatus`** (status do destino: melhor entre os sids do slug → `green`/`yellow`/`red`/null-não-registrado), **`toArmed`** (compat: `false` ⟺ `toStatus === 'red'`) e **`toPrio`** (prioridade do destino, default 1000) — o front mostra um countdown **global** (`próximo check`, todos checam no mesmo tique de 5 min), por card "na fila há Xm Ys" ou, se o destino está **offline** (`toStatus === 'red'`), um **✕ vermelho grande** à direita do card (o handoff vai ficar parado no inbox dele até voltar), e um **badge de prioridade** sempre ao lado do destino (`↓` cede a vez / `1000` neutro / `↑` fura a fila). A inbox é ordenada por **prioridade** (maior em cima), depois por tempo de espera. No topo do state, **`cronInterval`** = o intervalo do cron em minutos (config global `<base>/.bus-cron-interval`, default 5) — o front usa no countdown "próximo check" e o stepper "check" grava via `POST /api/cron-interval`. **Escritas do dashboard** (fora isso, é read-only sobre os handoffs): `POST /api/pause` (marcador `<projeto>/.bus-paused`), `POST /api/cron-interval` (`<base>/.bus-cron-interval`, 1–30) e `POST /api/cancel` (remove um handoff `from-operador` ainda no inbox).
