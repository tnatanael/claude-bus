...
- **`/bus-git-watch [owner/repo]`** — vigia as **issues de um repositório GitHub** e converte **cada evento** (issue nova, comentário, fechamento) num **handoff** pro especialista dono do assunto. A vigia é um **cron de hora em hora** rodando uma passada do `git-watch-tick.sh` — tarefa de fundo pendurada impede o tique do BUS de chegar na sessão (medido: 1 tique no dia contra 99–235 dos pares). É o par natural do BUS: **o BUS move o trabalho, o GitHub guarda backlog e histórico** — o que sobrevive ao `/clear`, ao restart e à compactação. Rode na sessão do **controlador (prio 0)**.
