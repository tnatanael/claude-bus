...
- **Entrega automática** — o cron de cada especialista (a cada 1 min) processa os handoffs sozinho; não precisa anunciar nem rodar `/bus` manual. Handoffs pra sessões offline esperam no dashboard.
...
Para mandar trabalho de uma sessão a outra, o especialista escreve um handoff endereçado ao slug do destino. O cron do destino (**a cada 1 min**, cron de sessão) processa sozinho — não precisa anunciar nem rodar `/bus` manual. O dashboard mostra os pendentes + quem está offline.
