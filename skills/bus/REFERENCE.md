...
- **Intervalo `*/5` (a cada 5 min):** barato — o gate defere tick vazio/bloqueado pré-API (zero API) e o jitter do harness dispersa os disparos, então o "herd" é pequeno. **Trade-off vs `*/1`:** mais latência pra pegar um handoff (até ~5 min + jitter) em troca de menos overhead de tique. É **configurável**: troque o `*/N` no `CronCreate` (SKILL passos 2/7 + bus-reload) se quiser outro intervalo.
