...
- **Intervalo `*/5` (a cada 5 min), FIXO pra toda a frota:** o cron é sempre `*/5 * * * *` — **NÃO há minuto nem offset por-sessão**. (O `bus-name` já emitiu um `BUS_CRON_MINUTE` determinístico por-sid pra "espalhar a frota", mas isso foi **removido**: era vestigial — o jitter do harness já dispersa os disparos e o gate serializa. Não tente casar nenhum minuto com o `*/5`; arme o `*/5 * * * *` e pronto.) Barato: o gate defere tick vazio/bloqueado pré-API (zero API). **Trade-off vs `*/1`:** mais latência pra pegar um handoff (até ~5 min + jitter) em troca de menos overhead. **Configurável:** troque o `*/N` no `CronCreate` (SKILL passos 2/7 + bus-reload) se quiser outro intervalo.
