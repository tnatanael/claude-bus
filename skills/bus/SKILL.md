...
7. **Ao encerrar o turno (PROCESSAR), nesta ordem:** (1) **re-arme o cron** — do zero, receita do passo 2 (`CronList`→`CronDelete` nos `/bus`→`CronCreate(cron:"*/1 * * * *", prompt:"/bus", recurring:true)`); fazer no **FIM** (não no início) evita parar o trabalho em voo só pra re-armar quando um `/bus` te acorda no meio. (2) **LIBERE O LOCK — sempre** (mesmo sem processar): *liberar lock* (solta o lock global se for seu; no-op se não) → os outros voltam a trabalhar na hora, sem esperar o lease (30 min).
