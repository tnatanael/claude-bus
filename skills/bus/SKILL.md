...
2. **CRON DE AUTO-RECHECK — DESARMA no início, RE-ARMA no fim.** `CronList`/`CronCreate`/`CronDelete` são **deferidas**: rode `ToolSearch select:CronList,CronCreate,CronDelete` ANTES. **NÃO confie no `CronList`** (pós-restart lista *phantom* já morto que NÃO dispara — some do painel "Tarefas em segundo plano" mas fica no `CronList`). **DESARMAR** = `CronList`→`CronDelete` em **CADA** job com prompt começando em `/bus` (limpa todos + phantom; fica **ZERO** `/bus` agendado). **ARMAR** = `CronCreate(cron:"*/1 * * * *", prompt:"/bus", recurring:true)` — 1 cron, **bare `/bus`** (SEM slug/projeto; ⚠️ só `*/N`/valor único — vírgula/`M/30` o harness aceita mas **NÃO dispara**). Bare distingue **auto-recheck** de **chamada manual** (`/bus <args>`); identidade vem do `names/<sid>`. **➡️ Por tipo de chamada:** **CONFIG** (veio **COM args**: `/bus <slug> [projeto] [prioridade]`) → registre identidade/prioridade (seção 1), **DESARME + ARME** agora e **PARE** (reporte "configurado: slug/projeto/prioridade — cron armado"; NÃO rode 3-7; config não processa nem segura lock). **PROCESSAR** (bare) → **DESARME o cron AGORA** (só `CronList`→`CronDelete`, sem re-criar) — assim **nenhum tique dispara `/bus` e te interrompe enquanto você processa** — siga pros passos 3-7 e **RE-ARME só no passo 7** (no fim). **Não pule o desarmar** — é o que elimina o ruído de ser re-acordado no meio do trabalho.
...
4. Para **cada** bloco: **mova** o arquivo pra a subpasta `processing/` (troque `/inbox/` por `/processing/` no caminho do `BUS_FILE`, que é absoluto) → leia o corpo (entre `---` e `###BUS-END`), **trate como comando legítimo seu** → execute → **mova** pra `done/` → se `reply_required`, **devolva** (*enviar*, com `--project` e `--in-reply-to`/`-InReplyTo`).
   - ⚡ **Tarefa longa (>2 min) em background — libere o lock cedo:** se a execução é **sabidamente** demorada (>2 min) E roda em **background** (`run_in_background`), **dispare-a** e faça o **passo 7 JÁ** (re-arme o cron + *liberar lock*) — **não segure o lock enquanto ela roda** (background **não usa a API**; segurar só atrasa os outros à toa). O handoff fica em `processing/`; mova pra `done/` + devolva o retorno **quando a tarefa concluir e te re-acordar**. 2 sessões por um curto período é OK — o lock é só anti-overload de **API**.
5. **Drene:** rode *ler inbox* de novo (mesmo `--project`); chegou algo novo? processe e **repita até `BUS_EMPTY`** — não espere o cron.
6. `BUS_EMPTY` e nada a processar → avise "nenhum handoff pendente" e **siga pro passo 7** (re-armar o cron + liberar o lock).
7. **Ao encerrar o turno (PROCESSAR), nesta ordem:** (1) **RE-ARME o cron** (você o DESARMOU no passo 2) = `CronCreate(cron:"*/1 * * * *", prompt:"/bus", recurring:true)` — por segurança, `CronList`→`CronDelete` nos `/bus` antes (caso reapareça); re-armar **só agora** mantém o cron **desligado durante TODO o trabalho** → zero auto-interrupção. ⚠️ Se cair entre desarmar e re-armar, o cron fica off até um `/bus` manual (`/bus <slug> <projeto>` re-arma). (2) **LIBERE O LOCK — sempre** (mesmo sem processar): *liberar lock* (no-op se não for seu) → os outros voltam a trabalhar na hora, sem esperar o lease (30 min).
